...
| 지표 | 변경 전 | 변경 후 | 이유 |
|------|---------|---------|------|
| 총 HTTP 요청 수 | 로그인 요청 포함 (많음) | 로그인 1회만 (줄어듦) | 매 iteration 로그인이 사라짐 |
| `http_req_failed` rate | 로그인 실패도 포함 | 순수 API 실패만 측정 | 로그인이 측정 대상에서 제외 |
| `http_req_duration` | 로그인 응답 시간도 평균에 반영 | API 응답 시간만 반영 | 측정 노이즈 제거 |
| iteration 평균 시간 | 로그인 지연(~100-300ms) 포함 | 감소 | 로그인 시간이 빠짐 |
| iteration TPS | 낮음 | 증가 | 같은 VU로 더 많은 반복 가능 |

#### 달라지지 않는 것

- 각 API 엔드포인트의 **개별 응답 시간**은 동일 (서버 성능 자체는 변하지 않음)
- 서버가 받는 **API별 부하**는 동일
- **threshold 통과 여부**는 거의 동일 (오히려 로그인 노이즈가 빠져서 더 정확)

> 결과 수치가 달라지는 것은 서버 성능이 변한 것이 아니라, **로그인이라는 측정 노이즈가 제거되어 순수 API 성능만 정확하게 측정**하게 된 것이다.
