...
public interface ReferenceApi {
        ...
        @Operation(summary = "레퍼런스 수정", description = "레퍼런스 정보를 부분 수정합니다. 전달된 필드만 수정되며, 소유자만 수정 가능합니다.")
        @ApiResponses(value = {
        ...
        @Operation(summary = "레퍼런스 목록 조회", description = """
        ...
        swyp12.team9.server.global.common.dto.ApiResponse<PaginationUtils.Cursor.PageResponse<ReferenceResponse>> getReferences(
                        @Parameter(description = "조회 타입 (ALL: 전체, PUBLIC: 공개, PRIVATE: 비공개)", example = "ALL") @RequestParam(defaultValue = "ALL") ReferenceType type,
